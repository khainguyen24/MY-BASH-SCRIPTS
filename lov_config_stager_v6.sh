...
# 06/01/2021 updated to: create stage directory for 'customWatchlists' this included
# 6/02/2021 updated: removed creating the success and error directories.. leaning on configuration importer -error and -success commands to generated the subdirectories
...
echo -e "\n\e[35m********************************************************************"
echo -e "              ***  BI2R LOV & CONFIG STAGER TOOL ***              "
echo -e "********************************************************************\e[0m"
sleep 1
echo -en "\e[35mCreating LOV_CONFIG_DIR directories.."
sleep 1
Create_LOV_CONFIG_DIRD_dir="$(mkdir --parent /var/temp/LOV_CONFIG_DIR/{LOV/bewl,CONFIG})"
# Defining paths
LOV_BEWL_DIR=/var/temp/LOV_CONFIG_DIR/LOV/bewl/
LOV_ATTR_DIR=/var/temp/LOV_CONFIG_DIR/LOV
LOV_ATTR_LIST_DIR=/var/temp/LOV_CONFIG_DIR/LOV
LOV_CWL_DIR=/var/temp/LOV_CONFIG_DIR/
LOV_CONFIG_DIR=/var/temp/LOV_CONFIG_DIR/CONFIG/
echo -e "..done!\n"
# 3 #######################################################

# Function to create the LOV and CONFIG ARRAYS
...
function STAGE_LOV_CONFIG () {
  ...
  #find event_types.xml amd analysts.xml and move to the /var/temp/LOV_CONFIG_DIR/LOV/bewl

      sleep 1
      echo -n "Copying the LOV bewl files to the /var/temp/LOV_CONFIG_DIR/LOV/bewl directory.."
      ...
      sleep 1
      echo -n "Copying/renaming the BI2R config files to the /var/temp/LOV_CONFIG_DIR/CONFIG directory.."
...
#function to copy over the customWatchlists directory (holds cwlTypeData and exclusionaryRules and need to be imported with import-cwl-configs.groovy)
function STAGE_CWL () {
echo -n "Copying CWL..."
  find $BI2R_EXPORT_DIR -type d -name customWatchlists -exec cp -rp {} ${LOV_CWL_DIR} \;
  sleep 1
  echo -e "...done!\n"
  sleep 1
}
...
#change ownwership and permissions of /var/temp/LOV_CONFIG_DIR
function CHOWN_CHMOD () {
  echo -n "Changing ownership and permission on LOV_CONFIG_DIR directory ..."
  chown -R webadmin:webadmin /var/temp/LOV_CONFIG_DIR/
  chmod -R 775 /var/temp/LOV_CONFIG_DIR/
  sleep 1
  echo -e "...done!\n"
}
...
CHOWN_CHMOD
sleep 1
echo -en "All LOV and CONFIG from: $BI2R_EXPORT_DIR have been copied to:\e[0m \e[32m/var/temp/LOV_CONFIG_DIR/\e[0m\n"
echo -e "\n"
exit 0;
