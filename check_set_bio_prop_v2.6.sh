...
USAGE_MESSAGE="$(echo 'To set bio.prop values for BI2R or I2AR... run the script agian use: check | bi2r | i2ar | reindex-bids-107.1 | reindex-bids-108.1 | reindex-tsdb-sensors | trans')"
...
# Disabling the following since when puppet lays down a new standalone-full-lab.xml, bio.prop and deployements dir we do not want to restore the older version while on a higher version
...
if [ -z "$1" ]
then
    echo -e "\e[35mMissing parm .. use: check | bi2r | i2ar | reindex-bids-107.1 | reindex-bids-108.1 | reindex-tsdb-sensors | trans\e[0m"
		sleep 1
		exit 1;
fi
#---------------------------
...
then
    echo -e "\e[35m\nSetting bio.prop values, standalone-full-lab.xml and deployemnts for i2ar ingest...\e[0m\n"
      ...
      sed -i -e 's/^iafis.ingestNoMatch=.*/iafis.ingestNoMatch=false/g' -e 's/^wl-merge.bypass=.*/wl-merge.bypass=false/g' -e 's/^syphon.originalImporter=.*/syphon.originalImporter=I2AR/g' -e 's/^ingest.systemMode=.*/ingest.systemMode=Normal/g' -e 's/^ingest-twpdes.bypass=.*/ingest-twpdes.bypass=false/g' -e 's/^ingest.clamd.enabled=.*/ingest.clamd.enabled=false/g' -e 's/^ingest.originalImporter=.*/ingest.originalImporter=I2AR/g' /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
		}
		set_properties_I2AR
		sleep 1
...
#4-Set bio.prop for "reindex-bids-107.1"
if [ $1 = "reindex-bids-107.1" ]
then
  #add properties to bio.prop for v107.1
  function ADD_v107_1_PROPERTIES () {
    echo -e "\e[35m********************************************************************"
    echo -e "   Adding Properties to the biometrics.properties file for v107.1        "
    echo -e "********************************************************************\e[0m"
    echo -e "\n*** The following properties were appended to the biometircs.properties file ***\n"
    echo "### added by check_set_bio_prop_v2.6.sh ###
ingest-orphan.enabled=false
ingest-twpdes.enabled=false
ingest-twpdes.bypass=true
ingest.originalImporter=I2AR
wl-merge.enabled=false" | tee -a /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
    echo -e "\n"
    sleep 1
  }
...
#---------------------
    ...
    function UNDEPLOY_WL-HITS-WAR () {
      ...
      echo -e "\e[35m...done!\e[0m\n"
    #Setting bio.prop  ingest-wlhits.enabled=false – SZ "ingest-wlhits.useBridgeQueue=false"
      echo -e "\e[35mSetting bio.prop ingest-wlhits.enabled=false and ingest-wlhits.useBridgeQueue=false ..\e[0m"
      sed -i -e 's/^ingest-wlhits.enabled=.*/ingest-wlhits.enabled=false/g' -e 's/^ingest-wlhits.useBridgeQueue=.*/ingest-wlhits.useBridgeQueue=false/g' /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
      grep 'ingest-wlhits.enabled=' /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
...
#---------------------
    ...
    function UNDEPLOY_WL-EXPORT-WAR () {
      ...
      echo -e "\e[35m...done!\e[0m\n"
    #Setting bio.prop  ingest-wlexport.enabled=false – SZ "ingest-wlexport.createWatchml=false"
      echo -e "\e[35mSetting ingest-wlexport.enabled=false and ingest-wlexport.createWatchml=false ..\e[0m"
      sed -i -e 's/^ingest-wlexport.enabled=.*/ingest-wlexport.enabled=false/g' -e 's/^ingest-wlexport.createWatchml=.*/ingest-wlexport.createWatchml=false/g' /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
      grep 'ingest-wlexport.enabled=' /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
...
##### Run Stuff within -reindex-bids-107.1 Switch #####
ADD_v107_1_PROPERTIES
#RESTORE_STANDALONE_FULL_LAB_ORIG (skipping this as we want to keep the v107.1 version)
...
#################################################################################################################
#################################################################################################################
####################################### Reindexing BIDs on v108.1 ###############################################
#################################################################################################################
#################################################################################################################
#4-Set bio.prop for BI2R  "reindex-bids-108.1"
...
then
    ...
    function ADD_v108_1_PROPERTIES () {
      ...
      echo "### added by check_set_bio_prop_v2.6.sh ###
ingest-twpdes.bypass=true" | tee -a /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
      echo -e "\n"
      sleep 1
    }
...
##### Run Stuff within -reindex-bids-v108.1 switch #####
...
then
  ...
  #) backing up the standalone-full-lab.xml before messing with it.."
      ...
      function SET_BIOPROP_FOR_TSDB_SENSORS () {
        ...
        sed -i -e 's/^ingest-twpdes.enabled=.*/ingest-twpdes.enabled=true/g' -e 's/^wl-merge.enabled=.*/wl-merge.enabled=true/g' -e 's/^ingest-wlexport.enabled=.*/ingest-wlexport.enabled=true/g' -e 's/^ingest-wlexport.createWatchml=.*/ingest-wlexport.createWatchml=true/g' /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
        sleep 1
        echo -e "\e[35m...done!\e[0m\n"
  		}
...
#################################################################################################################
###############         Config Settings for Transactional Ingest Server             #############################
#################################################################################################################
#---------------------
if [ $1 = "trans" ]
then
  echo -e "\e[35m*********************************************************************************************"
  echo -e "                 Configuration Settings for Transactional Ingest Server                           "
  echo -e "*********************************************************************************************\e[0m"
  sleep 1

#add properties to bio.prop
function ADD_BIOPROP_PROPERTIES () {
  echo -e "\e[35m********************************************************************"
  echo -e "  Adding Missing Properties to the biometrics.properties   "
  echo -e "********************************************************************\e[0m"
  echo -e "\n*** The following properties were appended to the biometircs.properties file they are in Paul's Rig for reindexing instructions but not present in the bio.prop ***\n"
  echo -e "\n### added by check_set_bio_prop_v2.6.sh ###
ingest-twpdes.bypass=true" | tee -a /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
  echo -e "\n"
  sleep 1
}

#reindexing bi2r General settings
function SET_BIOPROP_TRANS_INGEST_SERVER () {
  echo -e "\e[35mSetting bio.prop general settings for BI2R data..\e[0m"
  sed -i -e 's/^iafis.ingestNoMatch=.*/iafis.ingestNoMatch=true/g' -e 's/^wl-merge.bypass=.*/wl-merge.bypass=true/g' -e 's/^syphon.originalImporter=.*/syphon.originalImporter=BI2R/g' -e 's/^ingest.systemMode=.*/ingest.systemMode=Normal/g' -e 's/^ingest-twpdes.bypass=.*/ingest-twpdes.bypass=true/g' -e 's/^ingest.clamd.enabled=.*/ingest.clamd.enabled=false/g' -e 's/^ingest.originalImporter=.*/ingest.originalImporter=I2AR/g' -e 's/^ingest-orphan.enabled=.*/ingest-orphan.enabled=false/g' -e 's/^ingest-orphan.runDailyCheck=.*/ingest-orphan.runDailyCheck=false/g' -e 's/^ingest-wlhits.useBridgeQueue=.*/ingest-wlhits.useBridgeQueue=false/g' -e 's/^ingest-twpdes.enabled=.*/ingest-twpdes.enabled=true/g' -e 's/^wl-merge.enabled=.*/wl-merge.enabled=true/g' -e 's/^ingest-wlexport.enabled=.*/ingest-wlexport.enabled=true/g' -e 's/^ingest-wlexport.createWatchml=.*/ingest-wlexport.createWatchml=true/g' -e 's/^ingest-wlhits.enabled=.*/ingest-wlhits.enabled=false/g' -e 's/^ingest-atp.enabled=.*/ingest-atp.enabled=false/g' -e 's/^twpdes-merge.enabled=.*/twpdes-merge.enabled=false/g' /opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties
  sleep 1
  echo -e "\e[35m...done!\e[0m\n"
}
...
#RUN Stuff
ADD_BIOPROP_PROPERTIES
SET_BIOPROP_TRANS_INGEST_SERVER
#Check configurations after
CHECK_BIO_PROP
    echo -e "\n"
    echo -e "\e[35m\n*** NOTE: The bio.prop was set.. Disclaimer: NOT totally sure if all the settings are optimal for Transactional.. it's what i got to work with at this point!. ***\e[0m\n"
		sleep 1
		exit 0;
