...
#--------CHECK_GREP_PER_LINE - START-----------#
## This function will take what is returned from grep
## and make sure every line is the same
## good for making sure each item in a file is created with\
## the same attribute
function CHECK_GREP_PER_LINE () {
  #$1 absolute path of the file to check
  #$2 absolute Path to the log file example: /var/temp/epic_build_${1}_results.logs
  #$3 JIRA Number
  #$4 JIRA Description
...
if [[ "$#" -eq 6 ]]
then
	cat "$1" | grep "$5" | while read line ; do
		if [[ "$line" != "$6" ]]
		then
			fail_flag=2
#      echo "1st:if $fail_flag"
	  fi
  done
elif [[ "$#" -eq 7 ]]
then
	cat "$1" | grep "$5" | while read line ; do
      ...
      fail_flag=2
#      echo "2nd:if $fail_flag"
		fi
  done
fi

# echo "line:391 $fail_flag"
if [[ $fail_flag -eq 1 ]]
then
	echo -e "${3} ${PASSED} ${4}" >> ${2}
else
...
## ITWOAR-16031
## ITWOAR-16033
# ITWOAR-16034 *has issues with multiple grep lines being returned*
#* ITWOAR-16041
## ITWOAR-16044
## ITWOAR-16045
#* ITWOAR-16046
# ITWOAR-16049 *Got with Dan on clarification of what this Jira is doing*
## ITWOAR-16059
#* ITWOAR-16070 Set up ActiveMQ Broker RefClient
### v115.2 ###
## ITWOAR-16249 Change SAML singleSignOnServiceLocation for J mode servers
## ITWOAR-16241 Add ingest-solr.useBatchQueue to biometrics.properties
## ITWOAR-16240 Parameterize values within sso.properties
...
CHECK_VER "ITWOAR-15984" "Updates to servers for renaming "DEBUGGER" role as "OPERATOR" | Check: operatorWhitelist.txt" ${LOG_PATH} "ls -1 /opt/jboss/jboss-eap-7.0/modules/com/leidos/login/main/" "operatorWhitelist.txt"
...
# ITWOAR-16004
CHECK_FILE_PARAM "/opt/jboss/default/standalone/configuration/standalone-full-lab.xml" "${LOG_PATH}" "ITWOAR-16004" "Update standalone XML with new wlmerge bridge settings" "('TSDB', 'Sensor')"
...
## ITWOAR-16033 check attribs.json for citizenship property
CHECK_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/com/leidos/login/main/Attribs.json" "${LOG_PATH}" "ITWOAR-16033" "Add 1 new mapping to the Attribs.json(.erb) files for citizenship" "citizenship"

## ITWOAR-16034 check data.yaml for US CITIZENSHIP ******this has issues because it returns the grep for each user in the file******
CHECK_GREP_PER_LINE "/etc/govport/data.yaml" "${LOG_PATH}" "ITWOAR-16034" "check data.yaml for US CITIZENSHIP" 'citizenshipStatus' 'citizenshipStatus : "US"' '#     citizenshipStatus : "US"'


## ITWOAR-16041 This is a back end infrastructure change to stop management of the userwhitelist via puppet

## ITWOAR-16044 The solrBridge in standalone.xml needs to be updated, so that keys purged by sprocket scripts get indexed.
CHECK_FILE_PARAM "/opt/jboss/default/standalone/configuration/standalone-full-lab.xml" "${LOG_PATH}" "ITWOAR-16044" "Update solrBridge in standalone.xml" "state='Linked' OR state='Purged'"

## ITWOAR-16045
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16045" "Add properties to biometrics.properties" "Present" "S-MODE" "xdom-monitor.enabled=true"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16045" "Add properties to biometrics.properties" "Present" "S-MODE" "xdom-bundler.enabled=true"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16045" "Add properties to biometrics.properties" "Present" "S-MODE" "xdom-receiver.enabled=false"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16045" "Add properties to biometrics.properties" "Present" "S-MODE" "xdom-dashboard.enabled=true"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16045" "Add properties to biometrics.properties" "Present" "J-MODE" "xdom-monitor.enabled=false"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16045" "Add properties to biometrics.properties" "Present" "J-MODE" "xdom-bundler.enabled=false"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16045" "Add properties to biometrics.properties" "Present" "J-MODE" "xdom-receiver.enabled=true"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16045" "Add properties to biometrics.properties" "Present" "J-MODE" "xdom-dashboard.enabled=true"

## ITWOAR-16046 These properties have been parameterized through Foreman so they can be overridden

## ITWOAR-16049 This Jira is for steps to rerun and ingest scripts it may need look for files in  /var/i2ar/error got with Dan for clarification

## ITWOAR-16059 These properties have been parameterized through Foreman so they can be overridden default to True, but could be set to false in Foreman so just checking to make sure its there
CHECK_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16059" "Check biometrics.properties ingest-sg-events.notificationsEnabled is present can be true/false" 'ingest-sg-events.notificationsEnabled'

##v115.2 checks ###
## ITWOAR-16249 Change SAML singleSignOnServiceLocation for J mode servers
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/com/leidos/login/main/sso.properties" ${LOG_PATH} "ITWOAR-16249" "v115.2-Change SAML singleSignOnServiceLocation for J mode servers" "Present" "J-MODE" "sso.saml.singleSignOnServiceLocation=/sts-mock/startSsoX509/nocsv"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/com/leidos/login/main/sso.properties" ${LOG_PATH} "ITWOAR-16249" "v115.2-Change SAML singleSignOnServiceLocation for J mode servers" "Absent" "S-MODE" "sso.saml.singleSignOnServiceLocation=/sts-mock/startSsoX509/nocsv"

## ITWOAR-16241 Add ingest-solr.useBatchQueue to biometrics.properties
CHECK_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/mil/army/inscom/biometrics/resources/main/biometrics.properties" ${LOG_PATH} "ITWOAR-16241" "v115.2-Add ingest-solr.useBatchQueue to biometrics.properties" 'ingest-solr.useBatchQueue=false'

## ITWOAR-16240 Parameterize values within sso.properties
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/com/leidos/login/main/sso.properties" ${LOG_PATH} "ITWOAR-16240" "v115.2-Parameterize values within sso.properties. Checks: sso.saml.registrationId, sso.saml.relyingPartyEntityId, sso.saml.assertionConsumerServiceLocation" "Present" "J-MODE" "sso.saml.registrationId=stsMock" "sso.saml.relyingPartyEntityId=i2ar-samlSSO" "sso.saml.assertionConsumerServiceLocation=/saml-sp/login/saml2/sso/stsMock"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/com/leidos/login/main/sso.properties" ${LOG_PATH} "ITWOAR-16240" "v115.2-Parameterize values within sso.properties. Checks:sso.saml.assertingPartyEntityId, sso.saml.singleSignOnServiceLocation, sso.saml.assertingPartyVerificationCert, sso.saml.authInitLocation" "Present" "J-MODE" "sso.saml.assertingPartyEntityId=stsMock-startSsoX509" "sso.saml.singleSignOnServiceLocation=/sts-mock/startSsoX509/nocsv" "sso.saml.assertingPartyVerificationCert=stsMock.crt" "sso.saml.authInitLocation=/saml-sp/saml2/authenticate/stsMoc"

CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/com/leidos/login/main/sso.properties" ${LOG_PATH} "ITWOAR-16240" "v115.2-Parameterize values within sso.properties. Checks: sso.saml.registrationId, sso.saml.relyingPartyEntityId, sso.saml.assertionConsumerServiceLocation" "Absent" "S-MODE" "sso.saml.registrationId=stsMock" "sso.saml.relyingPartyEntityId=i2ar-samlSSO" "sso.saml.assertionConsumerServiceLocation=/saml-sp/login/saml2/sso/stsMock"
CHECK_MODE_FILE_PARAM "/opt/jboss/jboss-eap-7.0/modules/com/leidos/login/main/sso.properties" ${LOG_PATH} "ITWOAR-16240" "v115.2-Parameterize values within sso.properties. Checks:sso.saml.assertingPartyEntityId, sso.saml.singleSignOnServiceLocation, sso.saml.assertingPartyVerificationCert, sso.saml.authInitLocation" "Aresent" "S-MODE" "sso.saml.assertingPartyEntityId=stsMock-startSsoX509" "sso.saml.singleSignOnServiceLocation=/sts-mock/startSsoX509/nocsv" "sso.saml.assertingPartyVerificationCert=stsMock.crt" "sso.saml.authInitLocation=/saml-sp/saml2/authenticate/stsMoc"
...
####################################################################################################
# ITWOAR-16111 Add 1 new property to sso.properties(.erb) files for GWT-RPC URL regex
#* ITWOAR-16110 Update schema for watchlist core (canceled)
# ITWOAR-16121 Update schema for watchlist core
# ITWOAR-16141 Fix issue where user timeout is 1.2 hours instead of 12 hours
# ITWOAR-16134 Remove all references to uamt-users.yaml
# ITWOAR-16136 Add 'xdom.tranmit.adapter.statusMaxRecentBundles' properties to xdom.props
# ITWOAR-16100 Add 'bundler.transferExpirationSeconds' and 'bundler.includeCheckFiles' properties to xdom.props
#* ITWOAR-16114 Add "Hadoop short-cut" library install/verify to Install Docs (TO DO)
#* ITWOAR-13846 Investigate/implement better SOLR watchlist core utilization for watchlist queues (feed into this ITWOAR-16121)
# ITWOAR-16166 Update sourceHigh.classifications to include new BAT-EFT type
# ITWOAR-16090 Update the cron job for run_auto_trans_imports.sh
...
CHECK_SERVER_MODE "$SERVER_MODE"
SERVER_MODE_GLOBE="${SERVER_MODE}"
CREATE_LOG_FILE 'v116'
#---- Support Functions END-----#
#################################################
###  RUN STUFF for the BUILD EPICS v116 HERE  ###
#################################################
## equals check has been added and verified working on dev-head-s
#* means it’s procedural change in foreman or somewhere other than the server being checked
# means the check for the epic Build jira has not been create or verified
