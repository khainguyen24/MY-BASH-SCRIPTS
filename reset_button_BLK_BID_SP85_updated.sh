...
# creating backup dir(update with the correct path to the post-processing files) with appeded build and date
  ...
  echo "Moving all the properties files from ~/modules/main directory.."
  cp -rp ${MODULES_MAIN_DIR}agencyOriMap.xml.15* ${FULL_PATH_DIR}
  cp -rp ${MODULES_MAIN_DIR}data-source-normalization.xml.15* ${FULL_PATH_DIR}
  cp -rp ${MODULES_MAIN_DIR}masking-rules.xml.15* ${FULL_PATH_DIR}
  cp -rp ${MODULES_MAIN_DIR}usp-rules.xml.15* ${FULL_PATH_DIR}
