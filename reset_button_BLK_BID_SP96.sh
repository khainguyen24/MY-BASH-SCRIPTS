...
# creating backup dir(update with the correct path to the post-processing files) with appeded build and date
  ...
  mv ${MODULES_MAIN_DIR}agencyOriMap.xml.15* ${MODULES_MAIN_DIR}agencyOriMap.xml

  mv ${MODULES_MAIN_DIR}CategoryDirectives.xml ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}CategoryDirectives.xml.15* ${MODULES_MAIN_DIR}CategoryDirectives.xml

  mv ${MODULES_MAIN_DIR}data-source-normalization.xml ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}data-source-normalization.xml.15* ${MODULES_MAIN_DIR}data-source-normalization.xml

  mv ${MODULES_MAIN_DIR}masking-rules.xml ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}masking-rules.xml.15* ${MODULES_MAIN_DIR}masking-rules.xml

  mv ${MODULES_MAIN_DIR}namesIgnoreFile.txt ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}namesIgnoreFile.txt.15* ${MODULES_MAIN_DIR}namesIgnoreFile.txt

  mv ${MODULES_MAIN_DIR}oriIgnoreFile.txt ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}oriIgnoreFile.txt.15* ${MODULES_MAIN_DIR}oriIgnoreFile.txt

  mv ${MODULES_MAIN_DIR}usp-rules.xml ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}usp-rules.xml.15* ${MODULES_MAIN_DIR}usp-rules.xml
