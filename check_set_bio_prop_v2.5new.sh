...
# to run script:
# ./check_set_bio_prop.sh [ check | bi2r | i2ar | reindex-bids-107.1 | reindex-bids-108.1 | reindex-tsdb-sensors ]
# change logs
...
USAGE_MESSAGE="$(echo 'To set bio.prop values for BI2R or I2AR... run the script agian use: check | bi2r | i2ar | reindex-bids-107.1 | reindex-bids-108.1 | reindex-tsdb-sensors')"
...
if [ -z "$1" ]
then
    echo -e "\e[35mMissing parm .. use: check | bi2r | i2ar | reindex-bids-107.1 | reindex-bids-108.1 | reindex-tsdb-sensors\e[0m"
		sleep 1
		exit 1;
fi
#---------------------------
...
then
  ...
  function ADD_v107_1_PROPERTIES () {
    ...
    echo "### added by check_set_bio_prop_v2.5.sh ###
