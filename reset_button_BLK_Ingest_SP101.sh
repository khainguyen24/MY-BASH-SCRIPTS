...
then
    echo -e "\nMissing parameter...exiting.\n"
    echo "How to use this script:"
    echo "./reset_button_BLK_Ingest_SP101.sh <BLKBID_FOLDER> <SP##>"
    echo -e "\nExample:"
    echo "./reset_button_BLK_Ingest_SP101.sh 2019-12-04 SP101"
    echo -e "\n"
    exit 1;
    ...
    BUILD_NUMBER=$2
    BACKUP_STRING='BULK-INGEST_post-ingest_backup_'
...
# creating backup dir(update with the correct path to the post-processing files) with appeded build and date
  ...
  echo "Moving all the BLK Ingest post processing files now.."
  sleep 2
  ...
  mv ${AUTOMATED_SCRIPT_DIR}archive-stats.results ${FULL_PATH_DIR}
  mv ${AUTOMATED_SCRIPT_DIR}ingest.archives ${FULL_PATH_DIR}
  sleep 1
  # command to backup the properties files in ~/modules/main directory MODULES_MAIN_DIR
  echo "Moving all the properties files from ~/biometrics/resources/main directory and restoring the original backup.."
  mv ${MODULES_MAIN_DIR}agencyOriMap.xml ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}agencyOriMap.xml.1* ${MODULES_MAIN_DIR}agencyOriMap.xml

  mv ${MODULES_MAIN_DIR}CategoryDirectives.xml ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}CategoryDirectives.xml.1* ${MODULES_MAIN_DIR}CategoryDirectives.xml

  mv ${MODULES_MAIN_DIR}data-source-normalization.xml ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}data-source-normalization.xml.1* ${MODULES_MAIN_DIR}data-source-normalization.xml

  mv ${MODULES_MAIN_DIR}masking-rules.xml ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}masking-rules.xml.1* ${MODULES_MAIN_DIR}masking-rules.xml

  mv ${MODULES_MAIN_DIR}namesIgnoreFile.txt ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}namesIgnoreFile.txt.1* ${MODULES_MAIN_DIR}namesIgnoreFile.txt

  mv ${MODULES_MAIN_DIR}oriIgnoreFile.txt ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}oriIgnoreFile.txt.1* ${MODULES_MAIN_DIR}oriIgnoreFile.txt

  mv ${MODULES_MAIN_DIR}usp-rules.xml ${FULL_PATH_DIR}
  mv ${MODULES_MAIN_DIR}usp-rules.xml.1* ${MODULES_MAIN_DIR}usp-rules.xml
...
#Run stuff
BULK-INGEST_Backup $BLKBID_FOLDER $BUILD_NUMBER
echo "all done.. BLK Ingest post processing files backed up to: /var/$BACKUP_DIR"
